...
def image_downloader(extension):
    ...
    os.makedirs(os.getcwd()+'/Downloads/imgur', exist_ok=True)

    res = requests.get(url)
    res.raise_for_status()

    soup = bs4.BeautifulSoup(res.text, 'html.parser') # second argument passed to avoid the warning 
    ...
    for i, image in enumerate(image_elem):
        ...
        res.raise_for_status()
        image_file = open(os.path.join(os.getcwd()+'/Downloads/imgur',os.path.basename(image_url)), 'wb')
